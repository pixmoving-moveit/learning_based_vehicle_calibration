...
class NeuralNetworkSteering4(Node):
    class NeuralNetwork(nn.Module):
        def __init__(self):
            super(NeuralNetworkSteering4.NeuralNetwork, self).__init__()
    ...
    def __init__(self):
        super().__init__('neural_network_steering4')

        self.model = self.NeuralNetwork()

        
        data = pd.read_csv('steering_04.csv')
        dataa = pd.read_csv('steering_04.csv')
        ...
        self.FILTER_VEL = self.get_parameter('filter_vel').get_parameter_value().double_value
        self.FILTER_CMD = self.get_parameter('filter_cmd').get_parameter_value().double_value
        self.FILTER_ACC = self.get_parameter('filter_acc').get_parameter_value().double_value
        ...
        ax.set_title('Neural Network Output vs. Velocity and Throttling | Steering 4')
...
def main():
    rclpy.init()
    neural_network_steering4 = NeuralNetworkSteering4()
    rclpy.spin(neural_network_steering4)
